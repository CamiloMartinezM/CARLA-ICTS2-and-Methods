...
from .evaluate import compute_kde_nll, evaluate_multimodal
from .utils import post_process, print_info, viz_results
...
def do_val(cfg, epoch, model, dataloader, device, logger=None):
    ...
    info = f"loss_val:{loss_val:.4f}, \
            loss_goal_val:{loss_goal_val:.4f}, \
            loss_traj_val:{loss_traj_val:.4f}, \
            loss_kld_val:{loss_KLD_val:.4f}"
...
def inference(
    ...
    with torch.set_grad_enabled(False):
        ...
        if hasattr(all_distributions[0], "mus"):
            distribution = model.GMM(
                torch.cat([d.input_log_pis for d in all_distributions], axis=0),
        ...
        eval_results = evaluate_multimodal(
            all_pred_trajs,
            all_gt_trajs,
            mode=mode,
        ...
        for key, value in eval_results.items():
            info = f"Testing prediction {key}:{np.around(value, decimals=3)!s}"
            if hasattr(logger, "log_values"):
                logger.info(info)
            else:
                print(info)
        ...
        if test_mode:
            ...
            outputs = {
                ...
                "X_global": all_X_globals,
                "timestep": all_timesteps,
                "pred_trajs": all_pred_trajs,
                "gt_trajs": all_gt_trajs,
...
def inference_kde_nll(cfg, epoch, model, dataloader, device, logger=None):
    ...
    with torch.set_grad_enabled(False):
        ...
        nll_dict = (
            {"KDE_NLL": KDE_NLL}
            if cfg.MODEL.LATENT_DIST == "categorical"
            else {"KDE_NLL": KDE_NLL, "Goal_NLL": Goal_NLL}
        )
        info = f"Testing prediction KDE_NLL:{KDE_NLL:.4f}, per step NLL:{KDE_NLL_PER_STEP}"
        if hasattr(logger, "log_values"):
            logger.info(info)
        else:
            print(info)
