...
def compute_bounding_box(keypoints, video_resolution, return_discrete_values=True):
    ...
    left, right = (
        np.clip(left - extra_width, 0, width - 1),
        np.clip(right + extra_width, 0, width - 1),
    )
    top, bottom = (
        np.clip(top - extra_height, 0, height - 1),
        np.clip(bottom + extra_height, 0, height - 1),
    )
...
def extract_global_features(trajectories, extract_delta=False, use_first_step_as_reference=False):
    for trajectory in trajectories.values():
        trajectory.use_global_features(
            video_resolution=trajectory.resolution,
...
def assemble_ground_truth_and_reconstructions(
    anomaly_masks,
    trajectory_ids,
