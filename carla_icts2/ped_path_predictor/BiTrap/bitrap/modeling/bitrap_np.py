...
from bitrap.layers.loss import cvae_loss
...
class BiTraPNP(nn.Module):
    def __init__(self, cfg, dataset_name=None):
        ...
        self.q_z_xy = nn.Sequential(
            nn.Linear(self.hidden_size + self.cfg.GOAL_HIDDEN_SIZE, 128),
            nn.ReLU(),
            nn.Linear(128, 64),
            nn.ReLU(),
            nn.Linear(64, self.cfg.LATENT_DIM * 2),
        ...
        self.goal_decoder = nn.Sequential(
            nn.Linear(self.hidden_size + self.cfg.LATENT_DIM, 128),
            nn.ReLU(),
            nn.Linear(128, 64),
            nn.ReLU(),
        ...
        self.traj_dec_input_backward = nn.Sequential(
            nn.Linear(
