...
def draw_single_pose(img, pose, color=None):
    ...
    if isinstance(pose, np.ndarray):
        ...
        raise TypeError(f"Unknown pose type {type(pose)}")
    # plot points
    for point in pose:
        if point.max() > 0:
    ...
    for edge in edge_pairs:
        ...
        cv2.line(
            img,
            tuple(pose[edge[0]].tolist()),
            tuple(pose[edge[1]].tolist()),
            color=color,
            thickness=2,
        )
    return img
...
class Visualizer:
    ...
    def viz_distribution(self, dist, id_to_show, radius=3, thickness=3):
        ...
        if self.mode == "image":
            colors *= 255
        for i, (pi, mu, cov) in enumerate(zip(pis, mus, covariances, strict=False)):
            ...
            if self.mode == "plot":
                ell = mpl.patches.Ellipse(mu, v[0], v[1], 180.0 + angle, color=colors[i])
                ...
                cv2.ellipse(
                    overlay,
                    tuple(mu.astype(np.int32)),
                    (int(10 * v[1]), int(10 * v[0])),
                    180.0 + angle,
                    startAngle=0,
                    endAngle=360,
                    color=colors[i],
        ...
        if self.mode == "plot":
            pi_stats = f"Max pi:{pis.max():.3f},  Min pi:{pis.min():.3f} \nMean pi:{pis.mean():.3f}, STD pi:{pis.std():.3f}"
