...
class Event:
    ...
    def get_traj(self):
        ...
        for i in range(traj_num):
            ...
            time_stamp2 = 150 * i + 140
            if (
                self.moments[time_stamp2].game_clock - self.moments[time_stamp1].game_clock < -5.7
                or self.moments[time_stamp2].game_clock - self.moments[time_stamp1].game_clock
                > -5.5
            ):
                continue
        ...
        for i in range(len(all_all_player_locations)):
            ...
            ball_y = seq_data[:, -1, 1]
            if (
                np.max(ball_x) > Constant.X_MAX - Constant.DIFF
                or np.min(ball_x) < 0
                or np.max(ball_y) > Constant.Y_MAX
                or np.min(ball_y) < 0
            ):
                del_list.append(i)
        all_all_player_locations = np.delete(all_all_player_locations, del_list, axis=0)
