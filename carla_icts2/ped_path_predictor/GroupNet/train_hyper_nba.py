...
def train(train_loader, epoch):
    ...
    for data in train_loader:
        ...
        if iter_num % args.iternum_print == 0:
            print(
                f"Epochs: {epoch:02d}/{args.num_epochs:02d}| It: {iter_num:04d}/{total_iter_num:04d} | Total loss: {total_loss.item():03f}| Loss_pred: {loss_pred:03f}| Loss_recover: {loss_recover:03f}| Loss_kl: {loss_kl:03f}| Loss_diverse: {loss_diverse:03f}",
            )
        iter_num += 1
...
""" dataloader """
train_set = NBADataset(obs_len=args.past_length, pred_len=args.future_length, training=True)
