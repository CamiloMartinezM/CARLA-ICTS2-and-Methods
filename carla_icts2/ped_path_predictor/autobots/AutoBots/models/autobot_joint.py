...
class AutoBotJoint(nn.Module):
    ...
    def __init__(
        ...
        for _ in range(self.L_dec):
            ...
            self.temporal_attn_decoder_layers.append(
                nn.TransformerDecoder(tx_decoder_layer, num_layers=2),
            )
            tx_encoder_layer = nn.TransformerEncoderLayer(
                d_model=self.d_k,
                nhead=self.num_heads,
            ...
            self.social_attn_decoder_layers.append(
    ...
    def temporal_attn_fn(self, agents_emb, agent_masks, layer):
        ...
        agent_masks[:, -1][agent_masks.sum(-1) == T_obs] = (
            False  # Ensure agent's that don't exist don't throw NaNs.
        )
        agents_temp_emb = layer(
            self.pos_encoder(agents_emb.reshape(T_obs, B * (self._M + 1), -1)),
    ...
    def temporal_attn_decoder_fn(self, agents_emb, context, agent_masks, layer):
        ...
        agent_masks = agent_masks.permute(0, 2, 1).reshape(-1, T_obs)
        agent_masks[:, -1][agent_masks.sum(-1) == T_obs] = (
            False  # Ensure that agent's that don't exist don't make NaN.
        )
    ...
    def forward(self, ego_in, agents_in, roads, agent_types):
        ...
        if self.use_map_lanes:
            ...
            map_features = (
                ...
                .repeat(1, 1, self.c, 1, 1)
                .view(-1, B * self.c * (self._M + 1), self.d_k)
            )
            road_segs_masks = (
                orig_road_segs_masks.unsqueeze(2)
                .repeat(1, self.c, 1, 1)
                .view(B * self.c * (self._M + 1), -1)
            )

        # Repeat the tensors for the number of modes.
        opps_masks_modes = (
            opps_masks.unsqueeze(1).repeat(1, self.c, 1, 1).view(B * self.c, ego_in.shape[1], -1)
        )
        ...
        agent_types_features = (
            self.emb_agent_types(agent_types)
            .unsqueeze(1)
            .repeat(1, self.c, 1, 1)
            .view(-1, self._M + 1, self.d_k)
        )
        ...
        mode_probs = (
            self.prob_predictor(mode_params_emb)
            .squeeze(-1)
            .view(self.c, B, self._M + 1)
            .sum(2)
            .transpose(0, 1)
        )
