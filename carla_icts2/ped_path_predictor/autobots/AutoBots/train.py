...
class Trainer:
    def __init__(self, args, results_dirname):
        ...
        self.optimiser = optim.Adam(
            self.autobot_model.parameters(), lr=self.args.learning_rate, eps=self.args.adam_epsilon,
        )
        self.optimiser_scheduler = MultiStepLR(
            self.optimiser, milestones=args.learning_rate_sched, gamma=0.5, verbose=True,
        )
    ...
    def initialize_dataloaders(self):
        if "Nuscenes" in self.args.dataset:
            train_dset = NuscenesH5Dataset(
                dset_path=self.args.dataset_path,
                split_name="train",
            ...
            train_dset = InteractionDataset(
                dset_path=self.args.dataset_path,
                split_name="train",
                use_map_lanes=self.args.use_map_lanes,
                evaluation=False,
            )
            val_dset = InteractionDataset(
                dset_path=self.args.dataset_path,
                split_name="val",
                use_map_lanes=self.args.use_map_lanes,
                evaluation=False,
            ...
            train_dset = ArgoH5Dataset(
                dset_path=self.args.dataset_path,
                split_name="train",
                use_map_lanes=self.args.use_map_lanes,
            )
            val_dset = ArgoH5Dataset(
                dset_path=self.args.dataset_path,
                split_name="val",
                use_map_lanes=self.args.use_map_lanes,
            )
    ...
    def initialize_model(self):
        if "Ego" in self.args.model_type:
            ...
            self.autobot_model = AutoBotJoint(
                k_attr=self.k_attr,
                d_k=self.args.hidden_size,
                _M=self.num_other_agents,
                c=self.args.num_modes,
    ...
    def _compute_ego_errors(self, ego_preds, ego_gt):
        ...
        fde_losses = (
            torch.norm(ego_preds[:, -1, :, :2] - ego_gt[:, -1, :, :2], 2, dim=-1)
            .transpose(0, 1)
            .cpu()
            .numpy()
        )
    ...
    def _compute_marginal_errors(self, preds, ego_gt, agents_gt, agents_in):
        ...
        agents_gt = (
            torch.cat((ego_gt.unsqueeze(2), agents_gt), dim=2).unsqueeze(0).permute(0, 2, 1, 3, 4)
        )
        error = (
            torch.norm(preds[:, :, :, :, :2] - agents_gt[:, :, :, :, :2], 2, dim=-1) * agent_masks
        )
    ...
    def _compute_joint_errors(self, preds, ego_gt, agents_gt):
        ...
        for k in range(self.args.num_modes):
            fde_error = (
                (
                    torch.norm(preds[k, -1, :, :, :2] - agents_gt[:, -1, :, :2], 2, dim=-1)
                    * agents_masks[:, -1]
                )
                .cpu()
                .numpy()
    ...
    def autobotego_train(self):
        ...
        for epoch in range(self.args.num_epochs):
            ...
            for i, data in enumerate(self.train_loader):
                ...
                if i % 10 == 0:
                    print(
                        i,
                        "/",
                        len(self.train_loader.dataset) // self.args.batch_size,
                        "NLL loss",
                        round(nll_loss.item(), 2),
                        "KL loss",
                        round(kl_loss.item(), 2),
                        "Prior Entropy",
                        round(torch.mean(D.Categorical(mode_probs).entropy()).item(), 2),
                        "Post Entropy",
                        round(post_entropy, 2),
                        "ADE+FDE loss",
                        round(adefde_loss.item(), 2),
                    )
            ...
            self.writer.add_scalar(
                f"metrics/Train minADE_{self.args.num_modes}", train_minade_c[0], epoch,
            )
            self.writer.add_scalar(f"metrics/Train minADE_{10}", train_minade_10[0], epoch)
            self.writer.add_scalar(f"metrics/Train minADE_{5}", train_minade_5[0], epoch)
            self.writer.add_scalar(f"metrics/Train minADE_{1}", train_minade_1[0], epoch)
            self.writer.add_scalar(
                f"metrics/Train minFDE_{self.args.num_modes}", train_minfde_c[0], epoch,
            )
            self.writer.add_scalar(f"metrics/Train minFDE_{1}", train_minfde_1[0], epoch)
    ...
    def autobotego_evaluate(self, epoch):
        ...
        with torch.no_grad():
            ...
            val_minade_10 = min_xde_K(
                val_ade_losses, val_mode_probs, K=min(self.args.num_modes, 10),
            )
            ...
            # Log val metrics
            self.writer.add_scalar(
                f"metrics/Val minADE_{self.args.num_modes}", val_minade_c[0], epoch,
            )
            self.writer.add_scalar(f"metrics/Val minADE_{10}", val_minade_10[0], epoch)
            self.writer.add_scalar(f"metrics/Val minADE_{5}", val_minade_5[0], epoch)
            self.writer.add_scalar(f"metrics/Val minADE_{1}", val_minade_1[0], epoch)
            self.writer.add_scalar(
                f"metrics/Val minFDE_{self.args.num_modes}", val_minfde_c[0], epoch,
            )
            self.writer.add_scalar(f"metrics/Val minFDE_{1}", val_minfde_1[0], epoch)
    ...
    def autobotjoint_train(self):
        ...
        for epoch in range(self.args.num_epochs):
            ...
            epoch_mode_probs = []
            for i, data in enumerate(self.train_loader):
                ego_in, ego_out, agents_in, agents_out, map_lanes, agent_types = (
                    self._data_to_device(data)
                )
                pred_obs, mode_probs = self.autobot_model(
                    ego_in, agents_in, map_lanes, agent_types,
                )

                nll_loss, kl_loss, post_entropy, adefde_loss = nll_loss_multimodes_joint(
                    pred_obs,
                    ego_out,
                    agents_out,
                    mode_probs,
                ...
                with torch.no_grad():
                    ...
                    epoch_marg_mode_probs.append(
                        mode_probs.unsqueeze(1)
                        .repeat(1, self.num_other_agents + 1, 1)
                        .detach()
                        .cpu()
                        .numpy()
                        .reshape(-1, self.args.num_modes),
                    )
            ...
            mode_probs = np.concatenate(epoch_mode_probs)
            train_minade_c = min_xde_K(
                epoch_marg_ade_losses, epoch_marg_mode_probs, K=self.args.num_modes,
            )
            train_minfde_c = min_xde_K(
                epoch_marg_fde_losses, epoch_marg_mode_probs, K=self.args.num_modes,
            )
            ...
            print(
                ...
                train_minfde_c[0],
                "\n",
                "Train Scene minADE c",
                train_sminade_c[0],
                "Train Scene minFDE c",
                train_sminfde_c[0],
            )

            # Log train metrics
            self.writer.add_scalar(
                f"metrics/Train Marg. minADE {self.args.num_modes}",
                train_minade_c[0],
                epoch,
            )
            self.writer.add_scalar(
                f"metrics/Train Marg. minFDE {self.args.num_modes}",
                train_minfde_c[0],
                epoch,
            )
            self.writer.add_scalar(
                f"metrics/Train Scene minADE {self.args.num_modes}",
                train_sminade_c[0],
                epoch,
            )
            self.writer.add_scalar(
                f"metrics/Train Scene minFDE {self.args.num_modes}",
                train_sminfde_c[0],
                epoch,
            )
            ...
            print(
                "Best Scene minADE c",
                self.smallest_minade_k,
                "Best Scene minFDE c",
                self.smallest_minfde_k,
            )
    ...
    def autobotjoint_evaluate(self, epoch):
        ...
        with torch.no_grad():
            ...
            val_mode_probs = []
            for i, data in enumerate(self.val_loader):
                ego_in, ego_out, agents_in, agents_out, context_img, agent_types = (
                    self._data_to_device(data)
                )
                pred_obs, mode_probs = self.autobot_model(
            ...
            val_minade_c = min_xde_K(
                val_marg_ade_losses, val_marg_mode_probs, K=self.args.num_modes,
            )
            val_minfde_c = min_xde_K(
                val_marg_fde_losses, val_marg_mode_probs, K=self.args.num_modes,
            )
            ...
            # Log train metrics
            self.writer.add_scalar(
                f"metrics/Val Marg. minADE {self.args.num_modes}", val_minade_c[0], epoch,
            )
            self.writer.add_scalar(
                f"metrics/Val Marg. minFDE {self.args.num_modes}", val_minfde_c[0], epoch,
            )
            self.writer.add_scalar(
                f"metrics/Val Scene minADE {self.args.num_modes}", val_sminade_c[0], epoch,
            )
            self.writer.add_scalar(
                f"metrics/Val Scene minFDE {self.args.num_modes}", val_sminfde_c[0], epoch,
            )
