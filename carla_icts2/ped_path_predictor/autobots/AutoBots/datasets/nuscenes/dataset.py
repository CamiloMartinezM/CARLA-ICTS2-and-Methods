...
class NuscenesH5Dataset(Dataset):
    ...
    def get_input_output_seqs(self, ego_data, agents_data):
        ...
        ego_out = ego_data[
            traj_len - self.pred_horizon : traj_len
        ]  # guaranteed to be shape(horizon, 3)
        ego_in = np.zeros((6, 3))
    ...
    def select_valid_others(self, agents_data, agent_types):
        ...
        for n in range(agents_data.shape[1]):
            ...
            dist_travelled.append(
                np.linalg.norm(
                    agents_data[idx_first_pt + int(agent_traj_len) - 1, n, :2]
                    - agents_data[idx_first_pt, n, :2],
                    axis=-1,
                ),
            )
        ...
        active_agents = (
            np.array(dist_travelled) > 3.0
        )  # * vehicle_agents[1:] * agents_with_atleast_2_ts # 3.0
    ...
    def get_agent_roads(self, roads, agents_in):
        ...
        # Ego Agent
        args_closest_roads = np.argsort(
            np.linalg.norm(np.array([[0.0, 0.0]]) - mean_roads, axis=-1),
        )
        ...
        for n in range(self.num_others):
            if agents_in[-1, n, 2]:
                args_closest_roads = np.argsort(
                    np.linalg.norm(agents_in[-1:, n, :2] - mean_roads, axis=-1),
                )
                if len(args_closest_roads) >= N:
        ...
        roads[:, :, :, 2][np.where(roads[:, :, :, 2] < 0.0)] += (
            2 * np.pi
        )  # making all orientations between 0 and 2pi
    ...
    def __getitem__(self, idx: int):
        ...
        if self.use_joint_version:
            if self.rtn_extras:
                extras = [
                    dataset["translation"][idx],
                    dataset["rotation"][idx],
                    dataset["scene_ids"][idx][2].decode("utf-8"),
                ]
                return (
                    in_ego,
                    out_ego,
        ...
        if self.rtn_extras:
            ...
            extras = [
                dataset["translation"][idx],
                dataset["rotation"][idx],
                dataset["scene_ids"][idx][0].decode("utf-8"),
                dataset["scene_ids"][idx][1].decode("utf-8"),
            ]
