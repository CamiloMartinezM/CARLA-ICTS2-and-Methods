...
from datasets.interaction_dataset.utils import (
    LL2XYProjector,
    Point,
...
class InteractionDataset(Dataset):
    def __init__(self, dset_path, split_name="train", evaluation=False, use_map_lanes=True):
        ...
        for osm_fname in roads_fnames:
            road_info = self.get_map_lanes(osm_fname, 0.0, 0.0)
            self.max_num_road_segs = max(self.max_num_road_segs, len(road_info))
    ...
    def get_map_lanes(self, filename, lat_origin, lon_origin):
        ...
        for way in e.findall("way"):
            ...
            if way_type == "curbstone":
                ...
                continue
            elif way_type == "road_border" or way_type == "guard_rail":
            ...
            self.max_num_pts_per_road_seg = max(self.max_num_pts_per_road_seg, len(x_list))
        ...
        new_roads = np.zeros(
            (160, self.max_num_pts_per_road_seg, 6),
        )  # empirically found max num roads is 157.
    ...
    def split_input_output_normalize(self, agents_data, meta_data, agent_types):
        ...
        agent_masks = np.expand_dims(agents_data[:, :, 0] != -1, axis=-1).astype(
            np.float32,
        )  # existence mask
        agents_data[:, :, :2] -= np.array(
    ...
    def rotate_agents(self, ego_in, ego_out, agents_in, agents_out, roads, agent_types):
        ...
        )  # +2 adding three dimensions for original positions and yaw
        new_ego_in[:, 3:] = ego_in
        new_ego_in[:, 2] = ego_in[:, 4] - ego_in[-1:, 4]  # standardized yaw.

        new_ego_out = np.zeros(
            (ego_out.shape[0], ego_out.shape[1] + 2),
        )  # adding two dimensions for original positions
        new_ego_out[:, 2:] = ego_out
        new_ego_out[:, 4] -= ego_in[-1:, 4]  # standardized yaw.

        new_agents_in = np.zeros(
            (agents_in.shape[0], agents_in.shape[1], agents_in.shape[2] + 3),
        )  # + 2
        new_agents_in[:, :, 3:] = agents_in
        new_agents_in[:, :, 2] = agents_in[:, :, 4] - agents_in[:, -1:, 4]  # standardized yaw.

        new_agents_out = np.zeros(
            (agents_out.shape[0], agents_out.shape[1], agents_out.shape[2] + 2),
        )
    ...
    def __getitem__(self, idx: int):
        ...
        if self.evaluation:
            ...
            return (
                model_ego_in,
                ego_out,
                model_agents_in.transpose(1, 0, 2),
                agents_out.transpose(1, 0, 2),
                roads,
                agent_types,
                ego_in,
                agents_in.transpose(1, 0, 2),
                original_roads,
                translations,
            )
