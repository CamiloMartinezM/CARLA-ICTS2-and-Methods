...
def data_to_tensor(ego_in, agents_in, agent_roads, agent_types):
    return (
        torch.from_numpy(ego_in).float().to(device).unsqueeze(0),
    ...
    for dataf in datafiles:
        ...
        for i, scene_id in enumerate(scene_ids):
            ...
            for agent_id in agent_ids:
                ...
                if (
                    len(agent_data["track_to_predict"].tolist()) == 0
                    or not agent_data["track_to_predict"].tolist()[0]
                ):
                    continue
                ...
                curr_agent_type = (
                    [1.0, 0.0] if "car" in agent_data["agent_type"].tolist()[0] else [0.0, 1.0]
                )
            ...
            scene_collisions, new_preds, vehicles_only = collisions_for_inter_dataset(
                pred_obs.cpu().numpy(),
            ...
            for n, agent_id in enumerate(scene_agent_ids):
                for t in range(new_preds.shape[1]):
                    row_data = [
                        ...
                        ((t + 1) * 100) + 1000,
                        1,
                        int(agent_id == interest_agent_id),
                    ]
