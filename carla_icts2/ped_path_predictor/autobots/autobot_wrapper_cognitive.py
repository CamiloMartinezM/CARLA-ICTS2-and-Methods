...
class AutoBotWrapperCog:
    def __init__(self, path=None):
        ...
        self.logger.info("Model: AutoBotEgo Cognitive as Agent")
    ...
    def transform(self, x, y):
        ...
        agents_in = torch.cat(
            (
                torch.concatenate((agent_in, ex_mask), dim=-1).unsqueeze(-2),
                torch.concatenate((cf_as_agent, ex_mask), dim=-1).unsqueeze(-2),
            ),
            dim=-2,
        ).cuda()
    ...
    def _compute_ego_errors(self, ego_preds, ego_gt, ego_in=None):
        with torch.no_grad():
            ...
            a, f = (
                torch.square(ego_preds[:, :, :, :2] - ego_gt[:, :, :, :2])
                .sum(-1)
                .sqrt()
                .sum()
                .item(),
    ...
    def train(self):
        ...
        for epoch in range(1000):
            ...
            if did_epoch_better:
                self.logger.info(
                    f"Epoch {epoch} was better than last best epoch({last_best_epoch})",
                )
                last_best_epoch = epoch
            if epoch - last_best_epoch > 10:
                self.logger.info(
                    f"Stopping training, no improvement in 10 epochs saved{last_best_epoch}",
                )
                break
...
if __name__ == "__main__":
    if "--test" in sys.argv:
        abw_eval = AutoBotWrapperCog(
            path="./_out/AutoBotWrapperCog/obs15_pred20/2024-07-11_20-42-44/model_114.pth",
        )
        ...
        print("INT")
        print(
            f"&AutoBot(+Cognitive) & {int_1_a:.4f}/{int_1_f:.4f} & {int_2_a:.4f}/{int_2_f:.4f} & {int_3_a:.4f}/{int_3_f:.4f} & {int_4_a:.4f}/{int_4_f:.4f} & {int_5_a:.4f}/{int_5_f:.4f} & {int_6_a:.4f}/{int_6_f:.4f} \\\\",
        )
        print("NON-INT")
        print(
            f"&AutoBot(+Cognitive) & {non_int_1_a:.4f}/{non_int_1_f:.4f} & {non_int_2_a:.4f}/{non_int_2_f:.4f} & {non_int_3_a:.4f}/{non_int_3_f:.4f} & {non_int_4_a:.4f}/{non_int_4_f:.4f} & {non_int_5_a:.4f}/{non_int_5_f:.4f} & {non_int_6_a:.4f}/{non_int_6_f:.4f} \\\\",
        )
