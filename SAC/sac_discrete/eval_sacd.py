...
class EvalSacdAgent(BaseAgent):
    ...
    def evaluate(self):
        ...
        for current_episode in range(1, num_episodes + 1):
            ...
            print(
                f"Velocity Goal: {velocity_goal / current_episode:.4f}, Exec time: {np.mean(exec_time_list):.4f}",
            )
            self.file.write(
                "Speed: {:.2f}m/s, Dist.: {:.2f}m, Return: {:.4f}".format(
                    info["ped_speed"], info["ped_distance"], episode_return,
        ...
        print(f"Log file written here: {self.filename}")
        print("-" * 60)
        print(
            round(num_accidents / current_episode, 2),
            " % ",
