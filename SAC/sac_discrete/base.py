...
class BaseAgent(ABC):
    def __init__(
        ...
        self.log_dir = (
            ...
            + "_start"
            + str(start_steps)
            + ("_gn" + str(self.max_grad_norm) if max_grad_norm is not None else "")
        )
        print(self.log_dir)
    ...
    def train_episode(self):
        ...
        self.writer.add_scalar("stats/dist", dist, self.steps)
        self.writer.add_scalar("stats/avg_dist", average_dist, self.steps)
        ...
        print(
            f"Total steps: {self.steps}, Episode steps: {episode_steps}, Reward: {episode_return:.4f}",
        )
        print(
            "Policy; ",
            action_count,
    ...
    def learn(self):
        assert (
            hasattr(self, "q1_optim")
            and hasattr(self, "q2_optim")
            and hasattr(self, "policy_optim")
            and hasattr(self, "alpha_optim")
        )
    ...
    def evaluate(self, mode="VALIDATION"):
        ...
        for current_episode in range(1, num_episodes + 1):
            ...
            print(
                f"Crash Rate: {num_accidents / current_episode:.2f}, Nearmiss Rate: {num_nearmisses / current_episode:.2f}, Goal Rate: {num_goals / current_episode:.2f}",
            )
            print(
                f"Velocity Goal: {velocity_goal / current_episode:.4f}, Exec time: {np.mean(exec_time_list):.4f},TTG: {np.mean(time_to_goal):.4f}",
            )
            self.file.write(
                "Speed: {:.2f}m/s, Dist.: {:.2f}m, Return: {:.4f}".format(
                    info["ped_speed"], info["ped_distance"], episode_return,
        ...
        print(f"Num steps: {self.steps:<5}  return: {mean_return:<5.1f}")
        print(f"Num steps: {self.steps:<5}  goal return: {total_goal}")
        print("-" * 60)
