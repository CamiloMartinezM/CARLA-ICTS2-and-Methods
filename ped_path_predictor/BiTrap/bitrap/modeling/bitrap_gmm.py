...
from bitrap.layers.loss import mutual_inf_mc
...
class BiTraPGMM(nn.Module):
    def __init__(self, cfg, dataset_name=None):
        ...
        self.gt_goal_encoder = nn.GRU(
            input_size=self.cfg.DEC_OUTPUT_DIM,
            hidden_size=32,
            bidirectional=True,
            batch_first=True,
        )
        ...
        self.dec_init_hidden_size = (
            self.hidden_size + self.cfg.LATENT_DIM if self.cfg.DEC_WITH_Z else self.hidden_size
        )

        self.enc_h_to_forward_h = nn.Sequential(
            nn.Linear(self.dec_init_hidden_size, self.cfg.DEC_HIDDEN_SIZE),
            nn.ReLU(),
        )
        self.traj_dec_input_forward = nn.Sequential(
            nn.Linear(self.cfg.DEC_HIDDEN_SIZE, self.cfg.DEC_INPUT_SIZE),
            nn.ReLU(),
        )
        ...
        self.traj_dec_input_backward_vel = nn.Sequential(
            nn.Linear(
                self.cfg.DEC_OUTPUT_DIM,  # 2 or 4
                self.cfg.DEC_INPUT_SIZE,
            ),
            nn.ReLU(),
        )

        self.traj_dec_input_backward = nn.Linear(self.cfg.DEC_OUTPUT_DIM, self.cfg.DEC_OUTPUT_DIM)
        self.traj_dec_backward = nn.GRUCell(
            input_size=self.dec_init_hidden_size + self.cfg.DEC_OUTPUT_DIM,
            hidden_size=self.cfg.DEC_HIDDEN_SIZE,
        )

        self.traj_output = nn.Linear(
            self.cfg.DEC_HIDDEN_SIZE * 2,  # merged forward and backward
    ...
    def categorical_latent_net(self, enc_h, cur_state, target=None, z_mode=False):
        if target is not None:
            ...
            target_h = F.dropout(target_h, p=0.25, training=self.training)
    ...
    def forward(
        ...
        if target_y is not None:
            ...
            loss_dict = {
                ...
                "loss_traj": traj_NLL,
                "loss_kld": KLD,
                "mutual_info_p": mutual_info_p,
                "mutual_info_q": mutual_info_q,
            }
            ...
            _, loc_gmm_traj, _ = self.get_eval_GMM(
                pred_traj_full["mus"],
                pred_traj_full["log_vars"],
                pred_traj_full["corrs"],
                per_step=True,
                full_dist=True,
            )
            loss_dict = {}
        ...
        loc_gmm_goal_viz = self.GMM(
            loc_gmm_goal_full.input_log_pis.detach(),
    ...
    def get_train_val_GMM(self, mu, log_var, corr, target, per_step=False):
        ...
        if per_step:
            ...
            loc_gmm_reverse = self.GMM(
                log_pi.flip(2),
                -loc_gmm.mus.flip(2),
                loc_gmm.log_sigmas.flip(2),
                loc_gmm.corrs.flip(2),
            )
    ...
    def get_eval_GMM(self, mu, log_var, corr, h_x=None, per_step=False, full_dist=False):
        ...
        if not full_dist:
            if per_step:
                ...
                mu, log_var, corr = (
                    mu.permute(0, 2, 1, 3),
                    log_var.permute(0, 2, 1, 3),
                    corr.permute(0, 2, 1, 3),
                )
        ...
        if not per_step:
            ...
            loc_gmm_full = self.GMM(
                log_pi_full,
                mu_full.unsqueeze(1),
                log_var_full.unsqueeze(1),
                corr_full.unsqueeze(1),
            )
        else:
            predictions = (
                predictions.permute(0, 2, 1, 3) if not full_dist else predictions.squeeze(1)
            )
    ...
    def pred_future_traj_GMM(self, dec_h, goal_loc_gmm, K=25):
        ...
        flatten_goal_dist = self.GMM(
            torch.reshape(goal_loc_gmm.input_log_pis, [-1, 1]),
        ...
        for t in range(pred_len - 1, -1, -1):
            ...
            log_pis_t, mu_t, log_var_t, corrs_t = self.project_to_GMM_params_per_t(
