...
class GMM4D(td.Distribution):
    ...
    def __init__(self, log_pis, mus, log_sigmas, corrs):
        ...
        self.L = torch.stack(
            [
                torch.stack([sigma_0, zero_vector, zero_vector, zero_vector], dim=-1),
                torch.stack(
                    [
                        sigma_1 * rho_01,
                        sigma_1 * torch.sqrt(self.one_minus_rho2[..., 0]),
                        zero_vector,
                        zero_vector,
                    ],
                    dim=-1,
                ...
                torch.stack(
                    [
                        ...
                        zero_vector,
                        sigma_3 * rho_23,
                        sigma_3 * torch.sqrt(self.one_minus_rho2[..., 1]),
                    ],
                    dim=-1,
                ),
            ],
    ...
    def from_log_pis_mus_cov_mats(cls, log_pis, mus, cov_mats):
        ...
        sigmas = torch.stack(
            [torch.sqrt(sigma_1), torch.sqrt(sigma_2), torch.sqrt(sigma_3), torch.sqrt(sigma_4)],
            dim=-1,
        )
    ...
    def log_prob(self, value):
        r"""Calculates the log probability of a value using the PDF for bivariate normal distributions:
        ...
        xy_exp_nominator = (
            ...
            - 2
            * self.corrs[..., 0]
            * torch.prod(dx[..., :2], dim=-1)
            / torch.prod(self.sigmas[..., :2], dim=-1)
        )  # [..., N]
        ...
        xy_log_p = (
            -(
                ...
                + xy_exp_nominator / self.one_minus_rho2[..., 0]
            )
            / 2
        )
        ...
        wh_log_p = (
            -(
                ...
                + 2 * torch.sum(self.log_sigmas[..., 2:], dim=-1)
                + wh_exp_nominator / self.one_minus_rho2[..., 1]
            )
            / 2
        )
    ...
    def log_prob_component(self, value, idx):
        ...
        xy_log_p = (
            -(
                ...
                + xy_exp_nominator / one_minus_rho2[..., 0]
            )
            / 2
        )
        ...
        wh_log_p = (
            -(
                ...
                + 2 * torch.sum(log_sigmas[..., 2:], dim=-1)
                + wh_exp_nominator / one_minus_rho2[..., 1]
            )
            / 2
        )
    ...
    def get_covariance_matrix(self):
        ...
        E = torch.stack(
            [
                torch.stack([self.sigmas[..., 0] ** 2, cov_12, zeros, zeros], dim=-1),
