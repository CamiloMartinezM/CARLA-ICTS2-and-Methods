...
class SingleIntegrator(Dynamic):
    ...
    def integrate_distribution(self, v_dist):
        r"""Integrates the GMM velocity distribution to a distribution over position.
        ...
        if len(self.initial_conditions["pos"].shape) == 2:
            p_0 = (
                self.initial_conditions["pos"].unsqueeze(1).unsqueeze(1)
            )  # (Batch, dim) -> (Batch, 1, 1, dim)
        elif len(self.initial_conditions["pos"].shape) == 3:
            p_0 = self.initial_conditions["pos"].unsqueeze(
                1,
            )  # (Batch, component, dim) -> (Batch, 1, component, dim)
