...
def print_info(epoch, model, optimizer, loss_dict, logger):
    ...
    info = "Epoch:{},\t lr:{:6},\t loss_goal:{:.4f},\t loss_traj:{:.4f},\t loss_kld:{:.4f},\t \
        ...
        loss_dict["loss_traj"],
        loss_dict["loss_kld"],
        loss_dict["kld_weight"],
        loss_dict["z_logit_clip"],
    )
...
def viz_results(
    ...
    if y_global.shape[-1] == 2:
        ...
        viz.visualize(
            ...
            normalized=normalized,
            bbox_type=bbox_type,
            viz_time_step=list(range(0, T, 3)) + [-1],
        )
        viz.visualize(
            y_global[id_to_show],
            color=(255.0, 0, 0),
    ...
    if hasattr(dist_goal, "mus") and viz.mode == "plot":
        dist = {
            "mus": dist_goal.mus.numpy(),
            "log_pis": dist_goal.log_pis.numpy(),
            "cov": dist_goal.cov.numpy(),
        }
    ...
    if dim == 4:
        ...
        if hasattr(dist_traj, "mus") and cfg.DATASET.NORMALIZE != "none":
            ...
            if cfg.DATASET.NORMALIZE == "zero-one":
                A = torch.stack(
                    [
                        torch.stack([(_max - _min)[..., 0], zeros, zeros, zeros], dim=-1),
                ...
                A = torch.stack(
                    [
                        torch.stack([(_max - _min)[..., 0] / 2, zeros, zeros, zeros], dim=-1),
            ...
            try:
                traj_mus = torch.matmul(A.unsqueeze(2), dist_traj.mus.unsqueeze(-1)).squeeze(
                    -1,
                ) + b.unsqueeze(2)
                traj_cov = torch.matmul(A.unsqueeze(2), dist_traj.cov).matmul(
                    A.unsqueeze(2).transpose(-1, -2),
                )
                goal_mus = (
                    torch.matmul(A[:, 0:1, :], dist_goal.mus.unsqueeze(-1)).squeeze(-1)
                    + b[:, 0:1, :]
                )
                goal_cov = torch.matmul(A[:, 0:1, :], dist_goal.cov).matmul(
                    A[:, 0:1, :].transpose(-1, -2),
                )
            except:
                raise ValueError
