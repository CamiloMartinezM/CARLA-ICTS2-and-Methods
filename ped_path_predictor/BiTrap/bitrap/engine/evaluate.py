...
def evaluate_multimodal(
    ...
    if mode == "bbox":
        ...
        eval_results["per_step_displacement_error"] = (
            np.linalg.norm(pred_traj - tiled_target_traj, axis=-1).min(axis=2).mean(axis=0)
        )
    else:
        raise NameError("Wrong mode")
    if isinstance(distribution, (GMM2D, GMM4D)):
        if len(distribution.mus.shape) == 3:
            eval_results["Goal_NLL"] = (
                -distribution.log_prob(torch.tensor(target_traj[:, -1, :])).mean().item()
            )
            ...
            if isinstance(distribution, GMM2D):
                ...
                raise ValueError
            goal_dist = GMM(
                distribution.input_log_pis[:, -1],
                distribution.mus[:, -1],
                distribution.log_sigmas[:, -1],
                distribution.corrs[:, -1],
            )
            eval_results["Goal_NLL"] = (
                -goal_dist.log_prob(torch.tensor(target_traj[:, -1, :])).mean().item()
            )
            eval_results["Traj_NLL"] = (
                -distribution.log_prob(torch.tensor(target_traj)).mean().item()
            )
            mode_traj = np.array(distribution.mode())
            if target_traj.shape[-1] == 4:
                if bbox_type == "x1y1x2y2":
                    eval_results["mode_ADE"] = (
                        np.square(mode_traj - target_traj[:, :, None, :])
                        .mean(axis=(1, 3))
                        .min(axis=-1)
                        .mean()
                    )
                    eval_results["mode_CADE"] = (
                        np.square(
                            x1y1x2y2_to_cxcywh(mode_traj)[..., :2]
                            - x1y1x2y2_to_cxcywh(target_traj)[:, :, None, :2],
                        )
                        .mean(axis=(1, 3))
                        .min(axis=-1)
                        .mean()
                    )
                    eval_results["mode_FDE"] = (
                        np.square(mode_traj[:, -1] - target_traj[:, -1, None, :])
                        .mean(axis=-1)
                        .min(axis=-1)
                        .mean()
                    )
                    eval_results["mode_CFDE"] = (
                        np.square(
                            x1y1x2y2_to_cxcywh(mode_traj)[:, -1, :, :2]
                            - x1y1x2y2_to_cxcywh(target_traj)[:, -1, None, :2],
                        )
                        .mean(axis=-1)
                        .min(axis=-1)
                        .mean()
                    ...
                    eval_results["mode_ADE"] = (
                        np.square(
                            cxcywh_to_x1y1x2y2(mode_traj)
                            - cxcywh_to_x1y1x2y2(target_traj)[:, :, None, :],
                        )
                        .mean(axis=(1, 3))
                        .min(axis=-1)
                        .mean()
                    ...
                    eval_results["mode_CADE"] = (
                        np.square(mode_traj[..., :2] - target_traj[:, :, None, :2])
                        .mean(axis=(1, 3))
                        .min(axis=-1)
                        .mean()
                    )
                    eval_results["mode_FDE"] = (
                        np.square(
                            cxcywh_to_x1y1x2y2(mode_traj)[:, -1]
                            - cxcywh_to_x1y1x2y2(target_traj)[:, -1, None, :],
                        )
                        .mean(axis=-1)
                        .min(axis=-1)
                        .mean()
                    ...
                    eval_results["mode_CFDE"] = (
                        np.square(mode_traj[:, -1, :, :2] - target_traj[:, -1, None, :2])
                        .mean(axis=-1)
                        .min(axis=-1)
                        .mean()
                    )
...
def evaluate_bbox_traj(pred_traj, target_traj, bbox_type="x1y1x2y2"):
    ...
    if bbox_type == "x1y1x2y2":
        ...
        per_step_box_mse = np.square(
            cxcywh_to_x1y1x2y2(pred_traj) - cxcywh_to_x1y1x2y2(target_traj),
        )
    ...
    for batch_num in range(batch_size):
        for timestep in range(T):
            try:
                ...
                pdf = np.clip(
                    kde.logpdf(target_traj[batch_num, timestep].T),
