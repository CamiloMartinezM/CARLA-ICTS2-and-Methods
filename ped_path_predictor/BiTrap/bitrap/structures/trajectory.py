...
class Trajectory:
    ...
    def change_coordinate_system(self, video_resolution, coordinate_system="global", invert=False):
        if invert:
            if coordinate_system == "global":
                ...
                raise ValueError(
                    "Unknown coordinate system. Only global is available for inversion.",
                )
            ...
            raise ValueError(
                "Unknown coordinate system. Please select one of: global, bounding_box_top_left, or "
                "bounding_box_centre.",
            )
    ...
    def _from_image_to_bounding_box(coordinates, video_resolution, location="centre"):
        if location == "top_left":
            ...
            raise ValueError(
                "Unknown location for the bounding box. Please select either top_left or centre.",
            )
    ...
    def _from_image_to_centre_bounding_box(coordinates, video_resolution):
        ...
        for idx, kps in enumerate(coordinates):
            if any(kps):
                ...
                xs, ys = np.hsplit(kps.reshape(-1, 2), indices_or_sections=2)
                xs, ys = (
                    np.where(xs == 0.0, centre_x, xs) - centre_x,
                    np.where(ys == 0.0, centre_y, ys) - centre_y,
                )
    ...
    def input_missing_steps(self):
        ...
        while last_step_non_missing < trajectory_length - 1:
            step_is_missing = (
                np.sum(self.coordinates[last_step_non_missing + 1, :] == 0) == input_dim
            )
            ...
            if consecutive_missing_steps:
                ...
                for n in range(1, consecutive_missing_steps + 1):
                    a = (
                        (consecutive_missing_steps + 1 - n) / (consecutive_missing_steps + 1)
                    ) * start_trajectory
