...
class ETHUCYDataset(data.Dataset):
    def __init__(self, cfg, split):
        ...
        conf_json = open(cfg.DATASET.ETH_CONFIG)
        ...
        hyperparams["minimum_history_length"] = (
            self.cfg.MODEL.INPUT_LEN - 1 if self.split == "test" else 1
        )
        ...
        print(
            f"obs dist mu/sigma: {all_obs_distance.mean():.2f}/{all_obs_distance.std():.2f}, obs vel mu/sigma: {all_obs_vel.mean():.2f}/{all_obs_vel.std():.2f}, pred dist mu/sigma: {all_pred_distance.mean():.2f}/{all_pred_distance.std():.2f}, pred vel mu/sigma: {all_pred_vel.mean():.2f}/{all_pred_vel.std():.2f}",
        )
