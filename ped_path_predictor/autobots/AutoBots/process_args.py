...
def get_train_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        choices=["Argoverse", "Nuscenes", "trajnet++", "interaction-dataset"],
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        choices=["Autobot-Joint", "Autobot-Ego"],
    ...
    if "trajnet" in args.dataset:
        ...
        assert not args.use_map_image and not args.use_map_lanes, (
            "TrajNet++ has no scene map information..."
        )
        ...
        assert not args.use_map_image, (
            "Interaction-dataset does not have image-based scene information..."
        )
...
def create_results_folder(args):
    ...
    model_configname += (
        "_D"
        + str(args.num_decoder_layers)
        + "_TXH"
        + str(args.tx_hidden_size)
        + "_NH"
        + str(args.tx_num_heads)
    )
...
def load_config(model_path):
    ...
    with open(os.path.join(model_dirname, "config.json")) as fp:
        config = json.load(fp)
