...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="raw Dataset path to multi-agent folder.",
    )
    parser.add_argument(
        "--split-name",
        type=str,
...
if __name__ == "__main__":
    ...
    agent_trajectories = f.create_dataset(
        "agents_trajectories",
        shape=(num_scenes, max_num_agents, 40, 7),
        chunks=(1, max_num_agents, 40, 7),
        dtype=np.float32,
    )
    agent_types = f.create_dataset(
        "agents_types",
        shape=(num_scenes, max_num_agents, 2),
        chunks=(1, max_num_agents, 2),
        dtype=np.float32,
    )
