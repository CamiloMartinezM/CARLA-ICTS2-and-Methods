...
from nuscenes.prediction.helper import convert_global_coords_to_local, quaternion_yaw
...
class NuScenesDataset(Dataset):
    def __init__(
        ...
        self._number_future_road_points = (
            40  # the number of points to include along the current lane.
        )
    ...
    def _get_map_features(self, nusc_map, x, y, yaw, radius, reference_position):
        ...
        if len(relevant_pts) > 0:
            ...
            for i in range(len(relevant_pts)):
                indices = np.argsort(
                    np.linalg.norm(np.array([[x, y]]) - relevant_pts[i][:, :2], axis=1),
                )
            ...
            indices = np.argsort(
                np.linalg.norm(np.array([[x, y]]) - first_pts[:, :2], axis=1),
            ).tolist()
            ...
            for i in range(min(len(mymap), self._max_number_roads)):
                curr_map[i, : min(len(mymap[i]), self._number_future_road_points)] = mymap[i][
                    : min(len(mymap[i]), self._number_future_road_points)
                ]
        else:
    ...
    def choose_agents(self, past_samples, future_samples, sample_token):
        ...
        for key, value in past_samples.items():
            ...
            if (
                x_left <= value[0, 0] <= x_right
                and y_behind <= value[0, 1] <= y_infront
                and key in future_samples.keys()
                and useful_agent_bool
            ):
        ...
        agents_array = np.zeros(
            (self._number_closest_agents, (self.past_secs + self.future_secs) * 2, 3),
        )
    ...
    def __getitem__(self, idx: int):
        ...
        ego_array = np.zeros(
            ((self.future_secs + self.past_secs) * self.freq_hz, 3),
        )  # 3 for x,y,mask
        if len(rotated_ego_positions) > 18:
            print("too many timesteps...")
            rotated_ego_positions = rotated_ego_positions[
                : (self.future_secs + self.past_secs) * self.freq_hz
            ]
        ...
        raw_map = self._get_map_features(
            self._map_dict[map_name],
        ...
        for road_idx in range(len(raw_map)):
            for pt_idx in range(len(raw_map[road_idx])):
                if raw_map[road_idx, pt_idx, -1] == 1:
                    rotated_pt = convert_global_coords_to_local(
                        raw_map[road_idx, pt_idx, :2],
                        annotation["translation"],
                        annotation["rotation"],
                    )
                    if is_insquare(rotated_pt[0], [0.0, 0.0], self.ego_range):
