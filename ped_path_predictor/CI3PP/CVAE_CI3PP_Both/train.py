...
class CVAE_CI3PP_Both_Wrapper:
    def __init__(self, path=None):
        ...
        self.logger.info("Model: CVAE_CI3PP_Both")
        ...
        with torch.no_grad():
            for j, (x_val, y_val) in enumerate(dataloader):
                ...
                ade_losses, fde_losses, a, f = self._compute_ego_errors(
                    pred_obs.unsqueeze(0).transpose(1, 2), ego_out, ego_in=ego_in,
                )
...
if __name__ == "__main__":
    if "--test" in sys.argv:
        abw_eval = CVAE_CI3PP_Both_Wrapper(
            path="./ped_path_predictor/saved_models/15_20/c_ci3pp_both.pth",
        )
        ...
        print("INT")
        print(
            f"&CI3P+(Cognitive+Car+AS+CVAE) & {int_1_a:.4f}/{int_1_f:.4f} & {int_2_a:.4f}/{int_2_f:.4f} & {int_3_a:.4f}/{int_3_f:.4f} & {int_4_a:.4f}/{int_4_f:.4f} & {int_5_a:.4f}/{int_5_f:.4f} & {int_6_a:.4f}/{int_6_f:.4f} \\\\",
        )
        print("NON-INT")
        print(
            f"&CI3P+(Cognitive+Car+AS+CVAE) & {non_int_1_a:.4f}/{non_int_1_f:.4f} & {non_int_2_a:.4f}/{non_int_2_f:.4f} & {non_int_3_a:.4f}/{non_int_3_f:.4f} & {non_int_4_a:.4f}/{non_int_4_f:.4f} & {non_int_5_a:.4f}/{non_int_5_f:.4f} & {non_int_6_a:.4f}/{non_int_6_f:.4f} \\\\",
        )
    else:
