...
class PathPredictor:
    def __init__(
        self,
        model_path=None,
        observed_frame_num=observed_frame_num,
        ...
        log_dir = (
            "_out/m2p3/"
            + f"new_{epochs}_{batch_size}_all_seed_0_{self.observed_frame_num}_{self.predicting_frame_num}_"
            + str(time.time())
            + "/summary"
        )
        ...
        for epoch in range(epochs):
            ...
            for i in range(num_batches):
                ...
                if i % 100 == 0:
                    ...
                    print(
                        f"Epoch: {epoch + 1}, batch: {i} Loss: {recons_loss / 10:.4f}",
                    )
                    ckp_loss = 0
    ...
    def evaluate(self, x_test, y_test):
        with torch.no_grad():
            ...
            return (
                y_pred,
                torch.square(y_pred - y_test).sum(2).sqrt().sum().item(),
                self.fde(y_pred, y_test),
            )
...
if __name__ == "__main__":
    array = np.array(
        [
            [3.9918034076690674, 233.06858825683594],
            ...
            [1.7043260335922241, 233.33628845214844],
        ],
        dtype=np.float32,
    )
    ...
    print(f"Time taken: {time_taken / runs:.4f}ms")
    for node in res:
        print(node)
