...
from .MS_HGNN_batch import MLP, MS_HGNN_hyper, MS_HGNN_oridinary
...
class GroupNet(nn.Module):
    ...
    def forward(self, data):
        ...
        future_traj = (
            data["future_traj"]
            .view(batch_size * agent_num, self.args.future_length, 2)
            .to(device)
            .contiguous()
        )
        ...
        if self.args.learn_prior:
            ...
            if self.args.ztype == "gaussian":
                pz_distribution = Normal(
                    mu=torch.zeros(past_feature_repeat.shape[0], self.args.zdim).to(
                        past_traj.device,
                    ),
